 ...
 * @author Tom
 */
public class LIDASolverHierarchicalLowLevelOrdering implements Solver {
    ...
    boolean wholegrid = true; //false

    boolean debugging = false;
    boolean removeIntersections = false;
    int maxIntersectionRemovals = 100;

    public LIDASolverHierarchicalLowLevelOrdering(int width, int height, int maxLevels) {
        this(width, height, TSPGrid.randomGoals((int)Math.sqrt(width*height), width, height), maxLevels);
    }

    public LIDASolverHierarchicalLowLevelOrdering(int width, int height, GoalPoint[] goals, int maxLevels) {
        init(width, height, goals, maxLevels);
    }

    public LIDASolverHierarchicalLowLevelOrdering(GoalPoint[] goalpoints, int maxLevels) {
        int maxX = 0, maxY = 0;
        int width, height;
    ...
    public double solve() {            
        ...
        if (removeIntersections) {
            ...
            solution.addAll(solutionObjects[0]);
            int i = 0;

            while (PathConnector.hasIntersections(solution) && i < maxIntersectionRemovals) {
                solution = PathConnector.removeIntersection(solution);
                i++;
            }
            if (TSPGrid.goalObjectPathDistance(solution) < TSPGrid.goalObjectPathDistance(solutionObjects[0])) {
                solutionObjects[0] = new CopyOnWriteArrayList(solution);
                List<Integer> path = new ArrayList<Integer>();
                for (GoalObject go : solution) {
    ...
    public void backConnect(int level) {
        ...
        List<Integer> singleLevelPath = new ArrayList<Integer>();
        List<GoalObject> singleLevelGoalObjects = new ArrayList<GoalObject>();
        double cost = solveTsp(gridHierarchy[level - 1], gridHierarchy[level - 1].getGoalObjects(), singleLevelPath, singleLevelGoalObjects);

        int i = 0;
        for (int j = 0; j < singleLevelGoalObjects.size(); j++) {
            GoalObject lGoal = singleLevelGoalObjects.get(j);
            GoalObject hGoal = null;
            if (lowerLevelGoalObjects.size() >= singleLevelGoalObjects.size())
                break;
            for (int k = 0; k < highLevelGoals; k++) {
                if (solutionObjects[level].get(k).getSubGoalObjects().contains(lGoal)) {
                    hGoal = solutionObjects[level].get(k);
                    i = k;
                    while (true) {
                        if (j < singleLevelGoalObjects.size() - 1 && solutionObjects[level].get(k).getSubGoalObjects().contains(singleLevelGoalObjects.get(j+1)))
                            j++;
                        else break;
                    }
                    break;
            ...
            cost = solveTsp(gridHierarchy[level - 1], subTspGoals, subTspPath, subTspGoalObjects);
            solutionIterations += subSolutionIterations;
            //close path
            TSPGrid.closePath(subTspGoalObjects);
            subSolutionObjects[i] = subTspGoalObjects;
            subSolutionPaths[i] = subTspPath;
            if (lowerLevelGoalObjects.isEmpty()) { //first solution
                ...
                lowerLevelGoalObjects = PathConnector.mergePaths(lowerLevelGoalObjects, subTspGoalObjects, CONNECTIONK);
