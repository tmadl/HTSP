 ...
 * @author Tom
 */
public class LIDASolverHierarchicalSIExhaustive implements Solver {
    public static final double crossHierarchyDistance = 1, GOALAMPLIFICATION = 1;
    public static final int F = 3; //5 factor by which adjacent grid sizes differ (uneven!!!)
    ...
    public static final double CROSSLEVELK = 0.5; //0.5 1 used in cross level connection weight calculation
    public static final String CROSSCLBL = "CROSSC";

    public static final int CONNECTIONK = 100;//10;//, CONNECTIONZ = 5;
    ...
    int maxIntersectionRemovals = 50;

    @Override
    public void setTspDrawer(TspDrawer d) {drawer = d;}

    public void setTspDrawers(TspDrawer d1, TspDrawer d2) {
        drawer = d1; drawer2 = d2;

        if (GoalPath.debugging && gridHierarchy.length > 1) {
            Color[] colors = new Color[] {Color.CYAN, Color.MAGENTA, Color.ORANGE, Color.red, Color.green, Color.blue, Color.black, Color.pink, Color.yellow, Color.darkGray, Color.lightGray, new Color(120, 120, 120), new Color(160, 160, 160), new Color(200, 200, 200)};
            for (int j = 0; j < gridHierarchy[1].getGoalObjects().length; j++) {
                double centX = 0, centY = 0;
                for (GoalObject go : gridHierarchy[1].getGoalObjects()[j].getSubGoalObjects()) {
                    centX += go.getOriginalX();
                    centY += go.getOriginalY();
                }
                centX/=gridHierarchy[1].getGoalObjects()[j].getSubGoalObjects().size();
                centY/=gridHierarchy[1].getGoalObjects()[j].getSubGoalObjects().size();
                Color col;
                if (j < colors.length) {
                	col = colors[j];
                }
            	...
            	drawer.addPx(drawer.getwInc()+(int)Math.round(drawer.getwInc()*centX), drawer.gethInc()+(int)Math.round(drawer.gethInc()*centY), col);
            }
        }
    }

    public LIDASolverHierarchicalSIExhaustive(int width, int height, int maxLevels) {
        this(width, height, TSPGrid.randomGoals((int)Math.sqrt(width*height), width, height), maxLevels);
    }

    public LIDASolverHierarchicalSIExhaustive(int width, int height, GoalPoint[] goals, int maxLevels) {
        init(width, height, goals, maxLevels);
    }

    public LIDASolverHierarchicalSIExhaustive(GoalPoint[] goalpoints, int maxLevels) {
        int maxX = 0, maxY = 0;
        int width, height;
    ...
    public void setGoalPoints(GoalPoint[] goalpoints) {
...
/*
    ...
    public List<GoalObject> removeIntersections(List<GoalObject> goalObjectPath, int maxRemovals) {
        ...
        if (i >= maxRemovals) {
            System.err.println("failed to remove intersections ("+maxRemovals+")");
        }
...
*/
    ...
    public List<GoalObject> removeIntersections(List<GoalObject> lowerLevelGoalObjects) {
        boolean hasIntersections = PathConnector.hasIntersections(lowerLevelGoalObjects);
        int ri = 0;
        if (removeIntersections) {
            double prevcost = TSPGrid.goalObjectPathDistance(lowerLevelGoalObjects);
            List<GoalObject> prevObjects = lowerLevelGoalObjects;
            while (hasIntersections && ri < this.maxIntersectionRemovals) {
                hasIntersections = false;
                boolean breakfor = false;
                int pi = (int)(Math.random()*lowerLevelGoalObjects.size());
                int pj = (int)(Math.random()*lowerLevelGoalObjects.size());
                for (int ii = 0; ii < lowerLevelGoalObjects.size() && !breakfor; ii++) {
                    int i = (pi+ii)%lowerLevelGoalObjects.size();
                    if (i == 0) i++;
                    Edge edge = new Edge(lowerLevelGoalObjects.get(i-1), lowerLevelGoalObjects.get(i));
                    for (int ji = 0; ji < lowerLevelGoalObjects.size() && !breakfor; ji++) {
                        int j = (pj+ji)%lowerLevelGoalObjects.size();
                        if (j == 0) j++;
                        Edge cedge = new Edge(lowerLevelGoalObjects.get(j-1), lowerLevelGoalObjects.get(j));
                        if (edge.intersects(cedge)) { //intersection found
                            if (GoalPath.debugging) {
                                drawer.drawGoalObjectPath(lowerLevelGoalObjects, this.gridHierarchy[0].getNodes());
                                drawer.drawCircle(edge.getSource().getCurrentX(), edge.getSource().getCurrentY(), Color.orange, drawer.getGoalSize()+6);
                                drawer.drawCircle(edge.getSink().getCurrentX(), edge.getSink().getCurrentY(), Color.red, drawer.getGoalSize()+6);
                                drawer.drawCircle(cedge.getSource().getCurrentX(), cedge.getSource().getCurrentY(), Color.gray, drawer.getGoalSize()+6);
                                drawer.drawCircle(cedge.getSink().getCurrentX(), cedge.getSink().getCurrentY(), Color.black, drawer.getGoalSize()+6);

                            }
                            //edge swapping
                            List<GoalObject> newPath = new ArrayList<GoalObject>();
                            GoalPath gp = new GoalPath(lowerLevelGoalObjects);
                            gp.setDrawer(drawer);
                            newPath = gp.swapEdges(edge, cedge);
                            if (newPath.size() >= lowerLevelGoalObjects.size() && TSPGrid.goalObjectPathDistance(newPath) < TSPGrid.goalObjectPathDistance(lowerLevelGoalObjects))
                                lowerLevelGoalObjects = newPath;
                            else {
                                // kmin insertion
                                List<GoalObject> candidates = new ArrayList<GoalObject>();
                                candidates.add(edge.getSource());
                                candidates.add(edge.getSink());
                                candidates.add(cedge.getSource());
                                candidates.add(cedge.getSink());
                                //lowerLevelGoalObjects.removeAll(candidates);
                                for (GoalObject go : candidates) {
                                    lowerLevelGoalObjects.remove(go);
                                    List<GoalObject> candidate = new ArrayList<GoalObject>();
                                    candidate.add(go);
                                    lowerLevelGoalObjects = PathConnector.mergePaths(lowerLevelGoalObjects, candidate, CONNECTIONK);
                                }
                            }
                            if (GoalPath.debugging) {
                                drawer.drawGoalObjectPath(lowerLevelGoalObjects, gridHierarchy[0].getNodes());
                            }
            ...
            double newcost = TSPGrid.goalObjectPathDistance(lowerLevelGoalObjects);
            double gain = prevcost - newcost;
            //System.out.println(gain);
            if (gain <= 0)
                lowerLevelGoalObjects = prevObjects;
            else
                System.out.println("gained "+gain+" through ir");
            ri=ri;
        }
        return lowerLevelGoalObjects;
    }
    ...
    public void backConnect(int level) {
        ...
        /*
        //shuffled single-merge solution ~8% performance
        List<Integer> subTspPath = new ArrayList<Integer>();
        List<GoalObject> subTspGoalObjects = new ArrayList<GoalObject>();
        double cost = solveTsp(gridHierarchy[level - 1], gridHierarchy[level - 1].getGoalObjects(), subTspPath, subTspGoalObjects);
        Collections.shuffle(subTspGoalObjects);
        for (GoalObject go : subTspGoalObjects) {
            if (lowerLevelGoalObjects.isEmpty())
                lowerLevelGoalObjects.add(go);
            else {
                List<GoalObject> golist = new ArrayList<GoalObject>();
                golist.add(go);
                lowerLevelGoalObjects = PathConnector.mergePaths(lowerLevelGoalObjects, golist, CONNECTIONK);
            }
        }*/
        ...
        for (int i = 0; i < highLevelGoals; i++) {
            ...
            if (subTspGoalObjects.size() > maxClusterSize)
                maxClusterSize = subTspGoalObjects.size();
        }

        int n = 0;
        for (int j = 0; j < maxClusterSize; j++) {
            for (int i = 0; i < highLevelGoals; i++) {
                if (j >= subSolutionObjects[i].size())
                    continue;
                GoalObject cGoalObject = subSolutionObjects[i].get(j);
                if (lowerLevelGoalObjects.isEmpty())
                    lowerLevelGoalObjects.add(cGoalObject);
                else {
                    List<GoalObject> golist = new ArrayList<GoalObject>(), nearestOptions = new ArrayList<GoalObject>();
                    golist.add(cGoalObject);

                    /*consider only neighbors of goalobject
                    Set<GoalObject> nearestOptionsSet = new HashSet<GoalObject>();
                    for (Connection neighbor : gridHierarchy[level - 1].getNode(cGoalObject.getOriginalX(), cGoalObject.getOriginalY()).getNeighbors()) {
                        nearestOptionsSet.addAll((List<GoalObject>)neighbor.getSink().getContent());
                    }*/
                    //nearestOptionsSet.addAll(subSolutionObjects[(i-1+highLevelGoals)%highLevelGoals]);
                    //nearestOptionsSet.addAll(subSolutionObjects[(i+1+highLevelGoals)%highLevelGoals]);

                    //nearestOptions.addAll(nearestOptionsSet);
                    //lowerLevelGoalObjects = PathConnector.mergePathsUsingOptions(lowerLevelGoalObjects, golist, nearestOptions, CONNECTIONK);
                    lowerLevelGoalObjects = PathConnector.mergePaths(lowerLevelGoalObjects, golist, CONNECTIONK);

                    //if counting merge iterations
                    solutionIterations += PathConnector.mergeIterations;
                }
                n++;
                //debugging code
                if (GoalPath.debugging) {
                    if (level == 2) {
                        drawer2.drawCircle(cGoalObject.getCurrentX(), cGoalObject.getCurrentY(), Color.red, drawer2.getGoalSize()+4);
                        drawer2.drawGoalObjectPath(lowerLevelGoalObjects, gridHierarchy[level - 1].getNodes());
                    }
                    else if (level == 1) {
                        drawer.drawCircle(cGoalObject.getCurrentX(), cGoalObject.getCurrentY(), Color.red, drawer.getGoalSize()+4);
                        drawer.drawGoalObjectPath(lowerLevelGoalObjects, gridHierarchy[level - 1].getNodes());
                        drawer2.drawCircle(solutionObjects[level].get(i).getCurrentX(), solutionObjects[level].get(i).getCurrentY(), Color.red, drawer2.getGoalSize()+4);
                    }
                    try {
                        Thread.sleep(100);
                    }
        ...
        if (removeIntersections) {
            lowerLevelGoalObjects = removeIntersections(lowerLevelGoalObjects);
        }
        ...
        if (GoalPath.debugging) {
            if (level == 2) {
                drawer2.setPath(solutionPaths[level-1]);
                drawer2.draw();
            }
            else if (level == 1) {
    ...
    public double solveTsp(HSpatialGrid grid, GoalObject[] goals, int minX, int maxX, int minY, int maxY, List<Integer> resultPath, List<GoalObject> resultObjects) {
        ...
        if (removeIntersections) {
            resultObjects = removeIntersections(resultObjects);
            resultPath.clear();
            for (GoalObject go : resultObjects) {
                //solutionPaths[level-1].add(goalList.indexOf(go));
                GridNode gn = new GridNode(go.getCurrentX(), go.getCurrentY());
                resultPath.add(grid.getNodes().indexOf(gn));
            }
        }
