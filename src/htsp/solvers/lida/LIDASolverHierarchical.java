 ...
 * @author Tom
 */
public class LIDASolverHierarchical implements Solver {
    ...
    public static final int CONNECTIONK = 10, CONNECTIONZ = 5;
    ...
    public LIDASolverHierarchical(int width, int height, int maxLevels) {
        this(width, height, TSPGrid.randomGoals((int)Math.sqrt(width*height), width, height), maxLevels);
    }

    public LIDASolverHierarchical(int width, int height, GoalPoint[] goals, int maxLevels) {
        init(width, height, goals, maxLevels);
    }

    public LIDASolverHierarchical(GoalPoint[] goalpoints, int maxLevels) {
        int maxX = 0, maxY = 0;
        int width, height;
    ...
    protected final void init(int width, int height, GoalPoint[] goalpoints, int maxLevels) {
        ...
        boolean ed = TSPGrid.exactDistances;
    ...
    public void backConnect(int level) {
        ...
        for (int i = 0; i < highLevelGoals; i++) {
            ...
            if (i == 0) { //first solution
                ...
                ////List<GoalObject> mergeResult = PathConnector.mergePathsMultiedge(lowerLevelGoalObjects, subTspGoalObjects, CONNECTIONK, CONNECTIONZ);
